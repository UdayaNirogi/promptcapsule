...
class TestInterleavedOperations:
    ...
    def test_multiple_long_prompts(self):
        ...
        capsules = [self.pc.compress(p, vault_backend=self.backend) for p in prompts]
        results = [self.pc.decompress(c, vault_backend=self.backend) for c in capsules]
    ...
    def test_mixed_short_and_long(self):
        ...
        long_capsules = [self.pc.compress(p, vault_backend=self.backend) for p in long_prompts]
        ...
        # Decompress longs
        long_results = [self.pc.decompress(c, vault_backend=self.backend) for c in long_capsules]
